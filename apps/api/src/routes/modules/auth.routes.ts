...
const registerSchema = z.object({ fullName: z.string().min(2), email: z.string().email(), password: z.string().min(8), role: z.enum(["ADMIN", "VETERINARIAN", "NURSE", "RECEPTIONIST", "SHOP_STAFF"]) });
const loginSchema = z.object({ email: z.string().email(), password: z.string().min(8) });
const oauthSchema = z.object({ googleIdToken: z.string().min(10) });
...
authRouter.post("/oauth/google", async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json(parsed.error.flatten());
  try {
    return res.json(await oauthLogin(parsed.data.googleIdToken));
  } catch (error) {
    return res.status(401).json({ error: (error as Error).message });
  }
});
...
authRouter.post("/refresh", async (req, res) => {
  ...
  try {
    return res.json(await rotateRefreshToken(parsed.data.refreshToken));
  } catch (error) {
    return res.status(401).json({ error: (error as Error).message });
  }
});
...
authRouter.post("/password-reset/confirm", async (req, res) => {
  ...
  try {
    await resetPassword(parsed.data.resetToken, parsed.data.newPassword);
    return res.status(204).send();
  } catch (error) {
    return res.status(400).json({ error: (error as Error).message });
  }
